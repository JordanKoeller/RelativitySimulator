  ...
  fn poll(&mut self, time: Duration) -> bool {
    if let Some(last_time) = self.last_emitted {
      ...
      if time_since > self.max_interval {
        self.last_emitted = self.last_emitted.map(|le| le + (self.max_interval - self.min_interval) / 2u32); // Add the expectation value
        true
      } else if time_since > self.min_interval {
        let probability =
          (time_since - self.min_interval).as_secs_f32() / (self.max_interval - self.min_interval).as_secs_f32();
