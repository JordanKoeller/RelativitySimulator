...
use renderer::platform::{Uniform, Texture};
...
pub struct Shader {
  ...
  uniforms: RefCell<std::collections::HashMap<CString, i32>>,
}

impl Shader {
  pub fn new(name: &str, shader_body: &str) -> Shader {
    let shader_steps = Shader::decompress(shader_body.to_string());
    let program_id = Shader::compile_program(shader_steps);
    Shader {
      id: program_id,
      name: name.to_string(),
  ...
  pub fn from_file(name: &str, shader_path: &str) -> Shader {
    let file_body = super::shader_preprocessor::file_includer(shader_path);
    Shader::new(name, &file_body)
  }
...
pub fn set_uniform(&self, name: &CStr, unif: Uniform) {
  let mut unif_mgr = self.uniforms.borrow_mut();
  if let Some(&loc) = unif_mgr.get(name.clone()) {
    Shader::set_unif_helper(unif, loc);
  } else {
...
pub fn set_texture(&self, slot: u32, name: &CStr, texture: &Texture) {
  texture.bind(slot);
  ...
  fn compile_program(steps: Vec<ShaderStep>) -> u32 {
    unsafe {
      ...
      for step in steps.into_iter() {
        Shader::compile_shader(&mut program, step);
      }
