  ...
  E: Sync + Send + std::hash::Hash + Eq + Clone + std::fmt::Debug + 'static,
{
  ...
  fn for_each<F: FnMut(&E) -> ()>(&self, receiver: &ReceiverID, func: F) {
    self.inboxes[*receiver].iter().filter_map(|e| self.active_events.get(e)).for_each(func);
  }
...
P: Sized
{
  ...
  fn read(&self, receiver: &ReceiverID) -> Vec<&(E,P)> {
    self.inboxes[*receiver].iter().filter_map(move |subbed_evt| 
      self.active_events.get(subbed_evt)).flatten().collect()
  }

  fn for_each<F: FnMut(&(E,P)) -> ()>(&self, receiver: &ReceiverID, func: F) {
    self.inboxes[*receiver].iter().filter_map(move |subbed_evt| 
      self.active_events.get(subbed_evt)).flatten().for_each(func);
  }
