    ...
    fn allocate_buffer(&self, buf: &Vec<f32>, inds: &Vec<u32>, layout: Box<dyn GLLayout>) -> GLBuffer {
        ...
        unsafe {
            ...
            for i in 0..attrib_offsets.len() {
                // println!("Point({}, {}, FLOAT, FALSE, {}, {})", i, attrib_sizes[i], layout.elem_length(), attrib_offsets[i]);
                gl::VertexAttribPointer(
                    i as u32,
                    attrib_sizes[i],
                    gl::FLOAT,
                    gl::FALSE,
                    layout.elem_length() * size_of::<f32>() as i32,
                    (attrib_offsets[i] * size_of::<f32>() as i32) as *const i32 as *const c_void,
                );
