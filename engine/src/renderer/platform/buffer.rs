...
use crate::renderer::platform::BufferLayout;
...
impl DataBuffer {
    ...
    pub fn instancing_buffer(layout: BufferLayout, num_elems: u32) -> DataBuffer {
        let reservation: Vec<f32> = (0..(num_elems * layout.stride()) as usize)
            .into_iter()
            .map(|_| 0f32)
            .collect();
    ...
    fn validate_construct<E>(data: &[E], layout: BufferLayout, config: BufferConfig, id: u32) -> Self {
        ...
        let casted = unsafe { cast_slice::<E, f32>(data) };
...
impl DataBuffer {
    ...
    pub fn splice_inplace<F: FnOnce(&mut [f32]) -> ()>(&mut self, start: usize, end: usize, f: F) {
        f(self
            .data
            .get_mut(start..end)
            .expect("Tried to splice DataBuffer out of bounds"));
    ...
    pub fn splice_as<E: Sized, F: FnOnce(&mut [E]) -> ()>(&mut self, start: usize, end: usize, f: F) {
        ...
        // };
        let slice = unsafe { cast_slice_mut::<f32, E>(&mut self.data) };
