...
impl<'a> SystemDelegate<'a> for GameStateSystem {
        ...
        match data.game_state.state {
            GameStateEnum::Playing => {
                ...
                if game_over {
                    ...
                    data.game_state.score =
                        data.game_state.score + self.score_timer.start_poll_all(data.timestep.curr_time());
                }
            }
            _ => {}
        }
    ...
    fn update_debugger(&mut self, data: &mut Self::SystemData, debugger: &mut DebugPanel) {
        ...
        match (*data.game_state).state {
            GameStateEnum::Playing => {
                debugger.panel.lines[0] =
                    Box::from(LabeledText::new("Best Score", &data.game_state.best_score.to_string()));
